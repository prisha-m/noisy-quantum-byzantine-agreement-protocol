...
def main(name="n", runs=100):
    ...
    print(len(even_x), len(output_failure_prob))
    ...
    def plot_failure_probabilities(data, stderr, exact, title, filename):
        ...
        plt.xticks(ticks=even_x, labels=[f"{x:.3g}" for x in t1], rotation=45)
        plt.xlabel('T1 and T2 decoherence times (s)')
    ...
    plot_failure_probabilities(
        ...
        stderr,
        exact,
        f"Failure probability vs T1T2 decoherence noise - {subtitle(name)}",
        f"{name}_faulty.png"
    )
